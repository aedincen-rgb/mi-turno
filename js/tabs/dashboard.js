...
function DashboardTab(props){
  var calc=props.calc, turnos=props.turnos, salario=props.salario, vh=props.vh, ahora=props.ahora;
  ...
  var ctx=useMemo(function(){
    ...
    var turnosMes=turnos.filter(function(t){return new Date(t.inicio)>=ini;});
    var dias=calcPorDia(turnosMes, vh);
    var diasTrab=dias.length;
    var totalMins=calc.totalMins, totalCOP=calc.totalCOP;
    ...
    var festMins=(calc.bd.diurnaFest?.mins||0)+(calc.bd.noctFest?.mins||0)+(calc.bd.extraFestDiur?.mins||0)+(calc.bd.extraFestNoct?.mins||0);
    var noctMins=(calc.bd.noctOrd?.mins||0)+(calc.bd.extraNoct?.mins||0)+(calc.bd.noctFest?.mins||0)+(calc.bd.extraFestNoct?.mins||0);
    var proy=diaActual>0?(totalCOP/diaActual)*diasMes:0;
    var pctSalario=(totalCOP/salario)*100;
    return {dias:dias, diasTrab:diasTrab, diasMes:diasMes, diaActual:diaActual, prom:prom, promHoras:promHoras,
      mejor:mejor, festMins:festMins, noctMins:noctMins, proy:proy, pctSalario:pctSalario,
      totalMins:totalMins, totalCOP:totalCOP, turnosMes:turnosMes};
  },[calc, turnos, salario, vh, ahora.getMonth(), ahora.getDate()]);

  useEffect(function(){
    if(!canvasRef.current||!window.Chart) return;
    var tipos=Object.keys(calc.bd).filter(function(k){return calc.bd[k].mins>0;});
    if(tipos.length===0){
      if(chartRef.current){chartRef.current.destroy();chartRef.current=null;}
      return;
    }
    var labels=tipos.map(function(k){return RC[k].short;});
    var data=tipos.map(function(k){return Math.round(calc.bd[k].cop);});
    var colors=tipos.map(function(k){return RC[k].color;});
    ...
    chartRef.current=new window.Chart(canvasRef.current.getContext('2d'),{
      ...
      options:{
        ...
        plugins:{
          legend:{position:'bottom', labels:{color:isDark?'#7a7a8a':'#7a7a7a', font:{size:10,family:'DM Sans',weight:'600'}, padding:10, usePointStyle:true, boxWidth:7}},
          tooltip:{callbacks:{label:function(c){return ' '+c.label+': '+fCOP(c.raw);}}, backgroundColor:'rgba(20,20,24,0.94)', titleColor:'#d4ae74', bodyColor:'#fff', padding:10, cornerRadius:10}
        },
        cutout:'68%'
      }
    });
  ...
  var ritmoBadge='';
  if(ctx.pctSalario>=ctx.diaActual/ctx.diasMes*100*1.1) ritmoBadge='↑ Por encima del ritmo';
  else if(ctx.pctSalario>=ctx.diaActual/ctx.diasMes*100*0.9) ritmoBadge='✓ Buen ritmo';
  else ritmoBadge='↓ Por debajo del ritmo';

  var tip='';
  if(ctx.festMins>0) tip='Has trabajado <strong>'+fDur(ctx.festMins)+'</strong> en festivos, esos turnos pagan con recargo del 75% al 150%.';
  else if(ctx.noctMins>ctx.totalMins*0.4) tip='El '+((ctx.noctMins/ctx.totalMins)*100).toFixed(0)+'% de tus horas son <strong>nocturnas</strong>, eso eleva tu salario.';
  else tip='Con tu promedio de <strong>'+fCOP(ctx.prom)+'</strong> por turno, vas camino a <strong>'+fCOP(ctx.proy)+'</strong>.';

  var maxVal=Math.max.apply(null, ctx.dias.map(function(d){return d.cop;}).concat([1]));
  var CH=124;
  ...
  return h('div',{className:'fadeUp'},
    h('div',{className:'dash-hero'},
      ...
      h('div',{className:'dash-hero-sub'},'Basado en '+ctx.diasTrab+' día'+(ctx.diasTrab!==1?'s':'')+' · promedio '+fCOP(ctx.prom)),
      h('div',{className:'ritmo-badge'}, ritmoBadge)),

    h('div',{className:'dash-kpi-grid'},
      ...
      h('div',{className:'kpi-card'},
        ...
        h('div',{className:'kpi-sub'}, ctx.mejor?new Date(ctx.mejor.fecha+'T12:00:00').toLocaleDateString('es-CO',{day:'numeric',month:'short'}):'—'))),
    ...
    h('div',{className:'card'},
      ...
      h('div',{className:'bar-chart-wrap'},
        ctx.dias.map(function(d){
          ...
          return h('div',{key:d.fecha, className:'bar-col'},
            h('div',{className:'bar-fill-v '+cls, style:{height:hh+'px'}, title:d.fecha+': '+fCOP(d.cop)}),
            h('div',{className:'bar-label'}, d.fecha.slice(8)));
        }))),

    Object.keys(calc.bd).filter(function(k){return calc.bd[k].mins>0;}).length>0
      ?h('div',{className:'card'},

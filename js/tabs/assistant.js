...
function _aiLoadHistory() {
  try {
    var s = localStorage.getItem(_AI_HISTORY_KEY);
...
function AsistenteTab(props) {
  var ms = useState(_aiLoadHistory);
  var msgs = ms[0],
    setMsgs = ms[1];
  var is = useState('');
  var input = is[0],
  ...
  var clearChat = useCallback(function () {
    ...
    setMsgs([]);
    try {
      localStorage.removeItem(_AI_HISTORY_KEY);
    } catch (_) {
      /* ignore */
    }
  }, []);
      ...
      h(
        ...
        h(
          ...
          h(
            ...
            'Análisis local · 50+ funciones · redacta correos'
          )
        ),
        msgs.length > 0
          ? h(
        ...
        msgs.length === 0
          ? h(
              ...
              h(
                'div',
                null,
                'Pregúntame sobre tu mes o pídeme que redacte un correo.',
                h('br'),
                h(
                  'span',
                  { style: { fontSize: 11.5, opacity: 0.7 } },
                  'Sé responder 50+ cosas y mandar emails con tu reporte.'
                ),
                h('br'),
                h(
                  'span',
                  ...
                  'Prueba: ',
                  h('em', null, '"Envía mi reporte a juan@empresa.com"')
                )
              )
            )
          : msgs

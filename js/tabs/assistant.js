...
function EmailComposeCard(props){
  ...
  var canSend=cloud && to[0].trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to[0].trim()) && subject[0].trim() && body[0].trim();
  ...
  function doSend(){
    ...
    try{
      ...
      var fileBase64='';
      var filename='mi-turno-reporte-'+new Date().toISOString().slice(0,10)+'.'+(format[0]==='pdf'?'pdf':'xlsx');
      if(attach[0]){
        ...
        var calcAdapt={total:props.parent.calc.totalCOP, totalMin:props.parent.calc.totalMins};
        if(format[0]==='pdf') fileBase64=exportPDFBase64(turnosParaExport, calcAdapt, props.parent.salario);
  ...
  // Estados
  if(status[0]==='done'){
    return h('div',{className:'email-card email-card-done'},
      h('div',{className:'email-card-ico'},'✅'),
      h('div',{className:'email-card-msg'},
        h('strong',null,'Enviado'),' a ',h('span',{style:{color:'var(--accent)'}}, to[0])));
  }
  if(status[0]==='sending'){
    return h('div',{className:'email-card email-card-sending'},
      h('span',{className:'sp-in', style:{fontSize:18}}),
      h('span',{style:{color:'var(--muted)'}},'Enviando reporte...'));
  }

  return h('div',{className:'email-card'},
    h('div',{className:'email-card-hdr'},
    ...
    !cloud
      ?h('div',{className:'email-card-warn'},'⚠ Necesitas modo cloud activo para enviar correos.')
      :null,
    ...
    h('input',{
      ...
      onChange:function(e){to[1](e.target.value);}
    }),
    ...
    h('input',{
      ...
      className:'email-card-inp',
      value:subject[0],
      onChange:function(e){subject[1](e.target.value);}
    }),
    ...
    h('textarea',{
      ...
      value:body[0],
      rows:8,
      onChange:function(e){body[1](e.target.value);}
    }),

    h('div',{className:'email-card-opts'},
      h('label',{className:'email-card-opt'},
        h('input',{
          type:'checkbox',
          checked:attach[0],
          onChange:function(e){attach[1](e.target.checked);}
        }),
        h('span',null,'Adjuntar reporte')),
      attach[0]?h('div',{className:'email-card-fmt'},
        h('button',{
          className:'email-card-fmt-btn'+(format[0]==='pdf'?' on':''),
          onClick:function(){haptic();format[1]('pdf');}
        },'📄 PDF'),
        h('button',{
          className:'email-card-fmt-btn'+(format[0]==='xlsx'?' on':''),
          onClick:function(){haptic();format[1]('xlsx');}
        },'📊 Excel')):null),

    err[0]?h('div',{className:'email-card-err'}, err[0]):null,

    h('div',{className:'email-card-actions'},
      h('button',{className:'email-card-btn email-card-btn-ghost', onClick:discard},'Descartar'),
      h('button',{
    ...
    h('div',{className:'card'},
      h('div',{className:'ai-header'},
        ...
        h('div',{style:{flex:1,minWidth:0}},
          ...
          h('div',{style:{fontSize:11.5,color:'var(--muted)',marginTop:2}},'Análisis local · 50+ funciones · redacta correos')),
      ...
      h('div',{ref:winRef, className:'chat-window'},
        msgs.length===0
          ?h('div',{className:'ai-empty'},
              ...
              h('div',null,'Pregúntame sobre tu mes o pídeme que redacte un correo.',h('br'),
                h('span',{style:{fontSize:11.5,opacity:0.7}},'Sé responder 50+ cosas y mandar emails con tu reporte.'),h('br'),
                h('span',{style:{fontSize:11,opacity:0.55,marginTop:6,display:'inline-block'}},'Prueba: ',h('em',null,'"Envía mi reporte a juan@empresa.com"'))))
          :msgs.map(function(m,i){
              var bubble=h('div',{className:'chat-bubble', dangerouslySetInnerHTML:{__html:m.content.replace(/\n/g,'<br>').replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/`([^`]+)`/g,'<code style="padding:1px 6px;border-radius:6px;background:var(--accent-dim);color:var(--accent);font-size:0.92em">$1</code>').replace(/\*([^\*]+)\*/g,'<em>$1</em>')}});
              var actionCard=null;
              ...
              var resultBadge=m.actionResult?h('div',{className:'email-result-badge'+(m.actionResult.ok?' ok':' bad')}, m.actionResult.msg):null;
              return h('div',{key:i, className:'chat-msg '+m.role},
                h('div',{className:'chat-icon'}, m.role==='ai'?'✦':'·'),
                h('div',{style:{display:'flex',flexDirection:'column',gap:8,flex:1,minWidth:0,alignItems:m.role==='user'?'flex-end':'flex-start'}},
                  bubble,
                  actionCard,
                  resultBadge));
      ...
      h('div',{className:'chat-input-row'},
        h('textarea',{
          ...
          placeholder:'Pregunta o pide redactar un correo...',
          value:input,

...
function HomeTab(props){
  var calc=props.calc, activo=props.activo, ahora=props.ahora, vh=props.vh;
  ...
  if(activo&&vh){
    ...
    var factor=isHoliday?(isNight?2.10:1.75):(isNight?1.35:1.00);
  ...
  return h('div',{className:'fadeUp'},
    h('div',{className:'hero'},
      ...
      h('div',{className:'hero-amount'+(activo?' hero-amount-live':'')},fCOP(displayAmount)),
      ...
      h('div',{className:'action-stage'},
        h('button',{
          className:'action-btn '+(activo?'action-btn-stop':'action-btn-go'),
          onClick:function(){haptic();activo?props.onFin():props.onIni();}
        },
          h('div',{className:'action-icon'},activo?'■':'▶'),
          h('div',{className:'action-lbl'},activo?'Parar':'Iniciar')),
        activo
          ?h('div',{className:'active-box'},
              h('div',{className:'active-tag'},
                h('div',{className:'active-dot'}),
                'En turno'),
              h('div',{className:'active-timer'},fDur(durActual)),
              h('div',{className:'active-since'},
                'Desde '+new Date(activo.inicio).toLocaleTimeString('es-CO',{hour:'2-digit',minute:'2-digit'})),
      ...
      h('div',{className:'progress-block'},
        h('div',{className:'prog-row'},
          ...
          h('span',{className:'prog-val'+(activo?' prog-val-live':''),
            style:{color:pctMes>=100?'var(--success)':(activo?'var(--accent)':'var(--text)')}},
            pctMes.toFixed(1)+'%')),
        h('div',{className:'bar-track'},
          h('div',{className:'bar-fill'+(pctMes>=100?' bar-fill-over':'')+(activo?' bar-fill-live':''),
            style:{width:pctMes+'%'}})))),
    ...
    tipos.length===0
      ...
      :tipos.map(function(tipo){
          var val=calc.bd[tipo],r=RC[tipo];
          return h('div',{key:tipo,className:'brk-row',style:{'--brk-color':r.color}},
            h('div',{className:'brk-l'},
              h('div',{className:'brk-chip',style:{'--chip-bg':r.bg,'--chip-bd':r.bd,color:r.color}},r.icon),
              h('div',{className:'brk-meta'},
                h('div',{className:'brk-name'},r.label),
                h('div',{className:'brk-detail'},

...
function otpVerificar(input){
  ...
  if(Date.now()>_otpRef.expires){_otpRef.code=null;return false;}
  return input===_otpRef.code;
}

function otpLimpiar(){ _otpRef.code=null; _otpRef.expires=0; }

async function generarPINUnico(uid){
  var ts=Date.now(); var rnd=Math.floor(Math.random()*10000);

...
function aiInsightBreakdown(c) {
  ...
  // Horas nocturnas
  var noctMins = ((bd.noctOrd || {}).mins || 0) + ((bd.extraNoct || {}).mins || 0) +
    ((bd.noctFest || {}).mins || 0) + ((bd.extraFestNoct || {}).mins || 0);
  if (noctMins > 0) {
    var noctPct = c.totalMins > 0 ? (noctMins / c.totalMins * 100) : 0;
    parts.push('🌙 **' + (noctPct > 40 ? 'Alta carga nocturna' : 'Horas nocturnas') + ':** ' +
      fDur(noctMins) + ' (' + noctPct.toFixed(0) + '% del total). ' +
      (noctPct > 40
        ...
        : 'Las horas nocturnas pagan 35% extra. Si aumentaras un 10% tus horas de noche, ganarías aproximadamente ' + fCOP(c.totalCOP * 0.035) + ' adicionales este mes.'));
  }

  // Horas festivas
  var festMins = ((bd.diurnaFest || {}).mins || 0) + ((bd.noctFest || {}).mins || 0) +
    ((bd.extraFestDiur || {}).mins || 0) + ((bd.extraFestNoct || {}).mins || 0);
  if (festMins > 0) {
    parts.push('⛪ **Festivos trabajados:** ' + fDur(festMins) + ' en ' + (c.festDiasCount || 0) + ' día(s). ' +
      ...
      'Cada festivo trabajado suma aproximadamente ' + fCOP((c.salario || 0) / 30 * 0.75) + ' extra sobre un día normal.');
  }

  // Horas extra
  var extraMins = ((bd.extraDiurna || {}).mins || 0) + ((bd.extraNoct || {}).mins || 0) +
    ((bd.extraFestDiur || {}).mins || 0) + ((bd.extraFestNoct || {}).mins || 0);
  if (extraMins > 0) {
    parts.push('➕ **Horas extra:** ' + fDur(extraMins) + '. ' +
      ...
      (extraMins / 60 > 12
        ...
        : 'Aún tenés margen: podrías hacer hasta ' + (12 - extraMins / 60).toFixed(0) + 'h extra más esta semana sin exceder el límite legal.'));
  }
  ...
  if (c.totalMins > 0) {
    ...
    parts.push('📊 **Composición:** ' + ordPct.toFixed(0) + '% ordinario, ' + (100 - ordPct).toFixed(0) + '% con recargo. ' +
      (ordPct > 85
...
function aiInsightScenarios(c) {
  ...
  resp += '• ☀️ **Optimista:** ' + fCOP(optimista) + ' — Si mantenés el ritmo y metés horas extra o festivos.\n';
  ...
  if (falta > 0 && promDiario > 0) {
    ...
    resp += '\n🎯 Para alcanzar tu salario base de ' + fCOP(c.salario) + ', necesitás ≈ ' + diasParaMeta + ' día(s) más a tu ritmo actual.\n';
    if (promHoras > 0) {
      var horasDiarias = promHoras;
      resp += 'Si hicieras ' + (horasDiarias + 2).toFixed(0) + 'h en vez de ' + horasDiarias.toFixed(1) + 'h por día, llegarías en ≈ ' + Math.ceil(falta / (c.copPorHoraReal * (horasDiarias + 2))) + ' días.\n';
    }
  }
...
function aiInsightEfficiency(c) {
  ...
  var dias = c.dias;
  var mejor = dias.reduce(function (a, b) { return b.cop > a.cop ? b : a; }, dias[0]);
  var peor = dias.reduce(function (a, b) { return b.cop < a.cop ? b : a; }, dias[0]);

  var resp = '\n\n⚡ **Eficiencia**\n';

  // Mejor día
  if (mejor && mejor.fecha) {
    resp += '• 🏆 **Mejor día:** ' + _formatFecha(mejor.fecha) + ' — ' + fCOP(mejor.cop) + ' en ' + fDur(mejor.mins) + '\n';
  }

  // Peor día
  if (peor && peor.fecha && peor.fecha !== (mejor ? mejor.fecha : '')) {
    resp += '• 📉 **Día más bajo:** ' + _formatFecha(peor.fecha) + ' — ' + fCOP(peor.cop) + ' en ' + fDur(peor.mins) + '\n';
  }

  // COP por hora
  if (c.copPorHoraReal && c.vh) {
    var eficienciaPct = ((c.copPorHoraReal / c.vh - 1) * 100);
    resp += '• 💵 **Valor hora efectivo:** ' + fCOP(c.copPorHoraReal) + ' (' + (eficienciaPct >= 0 ? '+' : '') + eficienciaPct.toFixed(0) + '% sobre la base)\n';
    if (eficienciaPct > 20) {
      resp += '  ¡Excelente! Tus recargos están potenciando tu hora en más del 20%.\n';
    } else if (eficienciaPct < 5) {
      resp += '  Tus recargos son bajos. Considerá turnos nocturnos o festivos para aumentar este número.\n';
    }
  }
...
function aiInsightLegal(c) {
  ...
  if (c.hrsSemanales && c.hrsSemanales > 44) {
    parts.push('⚖️ **Jornada semanal:** ' + c.hrsSemanales.toFixed(1) + 'h. ' +
  ...
  if (c.rachaActual >= 6) {
    parts.push('⚠️ **Racha de ' + c.rachaActual + ' días:** La ley exige al menos 1 día de descanso por cada 6 trabajados (Art. 77 CST). ' +
  ...
  if (auxilioAplicable) {
    ...
    parts.push('🚌 **Auxilio de transporte:** Tenés derecho a $' + AUX_TRANSPORTE_2026.toLocaleString() + ' mensuales ' +
      '(≈$' + auxDiario.toFixed(0) + '/día). Esto aplica si ganás hasta 2 SMMLV. Activá esta opción en Ajustes > Preferencias.');
  }

  // Prestaciones
  if (c.totalCOP > 0) {
    var prestEst = c.totalCOP * 0.218;
    parts.push('💰 **Prestaciones estimadas:** ≈' + fCOP(prestEst) + ' (21.8% sobre bruto — incluye prima, cesantías, vacaciones e intereses). ' +
...
function aiInsightRecommendations(c) {
  ...
  if (c.pctSalario < 40 && c.diasRestantes > 5) {
    tips.push('💡 Vas al ' + c.pctSalario.toFixed(0) + '% de tu salario base. Para llegar al 100%, necesitás ≈' +
      fCOP((c.salario - c.totalCOP) * 1.1) + ' más. Probá trabajar fines de semana o noches — pagan entre 35% y 75% extra.');
  }

  // Si casi no tiene recargos
  var hasNocturno = ((c.bd && c.bd.noctOrd && c.bd.noctOrd.mins > 0) || (c.bd && c.bd.extraNoct && c.bd.extraNoct.mins > 0));
  var hasFestivo = ((c.bd && c.bd.diurnaFest && c.bd.diurnaFest.mins > 0) || (c.bd && c.bd.noctFest && c.bd.noctFest.mins > 0));
  if (!hasNocturno && !hasFestivo && c.diasTrab > 3) {
    tips.push('🌙 No has trabajado turnos nocturnos ni festivos este mes. Si hicieras 8h nocturnas una vez por semana, sumarías ≈' +
      fCOP(c.vh * 8 * 0.35 * 4) + ' extra al mes (35% de recargo).');
  }

  // Si la racha es larga
  if (c.rachaActual >= 5) {
    tips.push('🧘 Llevás ' + c.rachaActual + ' días sin descanso. Considerá tomarte un día libre — tu productividad y salud lo agradecerán. La ley además te protege.');
  }

  // Optimización de horario
  if (c.copPorHoraReal && c.vh && c.copPorHoraReal < c.vh * 1.1 && c.diasTrab > 3) {
    tips.push('📊 Tu valor hora efectivo es ' + fCOP(c.copPorHoraReal) + ', apenas un ' +
      (((c.copPorHoraReal / c.vh) - 1) * 100).toFixed(0) + '% sobre la base. Si movieras el 20% de tus horas a horario nocturno, ganarías ≈' +
      fCOP(c.totalCOP * 0.07) + ' extra sin trabajar más horas.');
  }
...
function aiInsightFull(c, intent) {
  ...
  // Solo agregar secciones según el intent
  if (intent === 'total_ganado' || intent === 'stats' || intent === 'proyeccion' || intent === 'analisis' || intent === 'distribucion') {
    ...
    sections.push(aiInsightEfficiency(c));
    sections.push(aiInsightRecommendations(c));
  } else if (intent === 'hoy' || intent === 'ayer') {
    sections.push(aiInsightEfficiency(c));
  }

  return sections.filter(function (s) { return s && s.length > 0; }).join('\n');
}

...
| v137-139 | **Barra inferior (home indicator):** `html::after` cubre la franja `env(safe-area-inset-bottom)` con color **sólido = último stop del degradado `--bg`** (`#f5f8ff` claro / `#040810` oscuro). Usar `var(--bg)` (gradiente) en esa franja reinicia el degradado y crea un escalón; el primer stop `#c8dfff` también. Override por `[data-theme='dark']` (no `@media`) para respetar el toggle in-app. |
| v158 | **Solución definitiva barra inferior:** La tab bar es una **píldora flotante idéntica al header** (`border-radius: 32px`, `margin-bottom: max(14px, var(--sab))`). La franja del home indicator se cubre con `html::after` usando los **mismos tokens glass** (`var(--glass-heavy)` + `var(--blur-xl)`) a `z-index: 99` (justo bajo la tab bar en 100). El vidrio continúa sin corte: la píldora flota sobre vidrio en vez de sobre el degradado claro. **NO cambiar** `border-radius` a `32px 32px 0 0` ni poner `margin-bottom: 0` — eso convierte la píldora en rectángulo pegado. NO volver a `var(--bg)` en `html::after` — eso expone el degradado blanco. Diseño estable. |
